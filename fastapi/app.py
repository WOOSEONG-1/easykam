...
def rag_search(query: str, topk: int = TOPK) -> Tuple[str, List[Dict[str, Any]]]:
    """
    질의 임베딩 → FAISS 상위 topk 검색 → 컨텍스트 블록 생성
    """
    assert FAISS_INDEX is not None and EMBED_MODEL is not None and DOCS is not None
...
def build_prompt_with_rag(q: str, history: List[Dict[str, Any]], context_block: str | None) -> str:
    ...
    """
    sys_inst = (
        "당신은 한국자산관리공사 내규 및 관련 제도를 설명하는 도우미입니다. "
        "사용자 질문에 대해, 제공된 참고 문서(있다면)를 우선 반영하고, "
        "관련 법령/사례는 링크 또는 정확 명칭을 함께 제시하세요. "
        "불확실하면 불확실하다고 말하세요."
    )
...
def ask(payload: AskIn, x_session_id: str = Header(default="")):
    ...
    # RAG가 아직 준비되지 않았다면 503 반환 (준비 전 502 방지)
    if FAISS_INDEX is None or EMBED_MODEL is None or not DOCS:
        raise HTTPException(503, "RAG 초기화 중입니다. 잠시 후 다시 시도해주세요.")
